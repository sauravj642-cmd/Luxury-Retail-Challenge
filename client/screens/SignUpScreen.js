...
export default function SignUpScreen({ navigation }) {
  ...
  const onSignUp = async () => {
    ...
    try {
      ...
      Alert.alert('Sign up failed', 'Please try a different email.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={{ flex: 1, padding: 24, gap: 12, justifyContent: 'center' }}>
      <Text style={{ fontSize: 24, fontWeight: '600', marginBottom: 16 }}>Create account</Text>
      ...
      <Button title={loading ? 'Creating...' : 'Sign Up'} onPress={onSignUp} />
    </View>
  );
}
